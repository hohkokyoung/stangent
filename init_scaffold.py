...
def update_gitignore(project_root: Path, dry_run: bool):
    ...
    entries_to_add = [
        ...
        ".stangent/.installed.json",
        ".stangent/symbol_index.json",
    ]
    entries_to_keep = [
...
def write_install_manifest(project_root: Path, dry_run: bool) -> None:
    """
    Write .stangent/.installed.json — a manifest of every file stangent installed.
    /uninit reads this to remove only stangent-owned files (no hardcoded lists).
    Future-proof: every new command, agent, or skill is auto-discovered at init time.
    """
    from datetime import datetime, timezone
    manifest_path = project_root / ".stangent" / ".installed.json"

    def _list_files(directory: Path, pattern: str = "*") -> list[str]:
        if not directory.exists():
            return []
        return sorted(
            str(f.relative_to(project_root).as_posix())
    ...
    manifest = {
        ...
        "files": {
            "commands":   _list_files(project_root / CLAUDE_COMMANDS_DIR,   "*.md"),
            "agents":     _list_files(project_root / CLAUDE_AGENTS_DIR,     "stangent*.md"),
            "subagents":  _list_files(project_root / CLAUDE_SUBAGENTS_DIR,  "stangent*.md"),
            "skills":     _list_files(project_root / CLAUDE_SKILLS_DIR,     "*.md"),
            "stangent_internal": [
                p for p in [
                    ".stangent/gateway/gateway.py",
                    ".stangent/observer/observer.py",
...
def configure_dbhub(config: dict, config_path: Path, project_root: Path, dry_run: bool, is_reinit: bool = False):
    ...
    Only prompts if not already enabled. Skipped silently on dry-run or reinit.
    """
    ...
    if is_reinit:
        return  # don't re-prompt on reinit; user can enable manually via config
...
def configure_supabase(config: dict, config_path: Path, profile_names: list, dry_run: bool, is_reinit: bool = False):
    """
    ...
    if is_reinit:
        return  # don't re-prompt on reinit; set integrations.supabase.enabled=true in config to activate
...
def _stangent_command_names() -> set[str]:
    """Discover stangent command filenames by scanning the stangent source.
    Replaces a hardcoded list so new commands are auto-handled by uninit.
    """
    src = STANGENT_PATH / "commands"
    return {f.name for f in src.glob("*.md")} if src.exists() else set()


def _load_install_manifest(project_root: Path) -> dict:
    """Read .stangent/.installed.json. Returns empty dict if absent or invalid."""
    p = project_root / ".stangent" / ".installed.json"
    if not p.exists():
        return {}
    try:
...
def _remove_command_files(project_root: Path, dry_run: bool) -> int:
    """Remove stangent-installed commands.
    Prefer manifest if available (definitive); fall back to source-dir scan.
    """
    count = 0
    ...
    manifest = _load_install_manifest(project_root)
    manifest_commands = manifest.get("files", {}).get("commands", [])

    if manifest_commands:
        names = {Path(p).name for p in manifest_commands}
    else:
        names = _stangent_command_names()
...
def _remove_gateway_files(project_root: Path, dry_run: bool):
    ...
    _remove_file(
        project_root / ".stangent" / ".installed.json",
        ".stangent/.installed.json",
        dry_run,
    )
