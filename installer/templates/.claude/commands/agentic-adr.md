...
1. Allocate id: `sh .claude/py .claude/hooks/lib/adr_id.py next` (e.g. `ADR-003`).
...
6. **Draft each ADR.** Allocate id with `sh .claude/py .claude/hooks/lib/adr_id.py next`. For each candidate write `.claude/adrs/<id>-<slug>.md`:
