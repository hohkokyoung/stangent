...
- Resolve the tester's model from `models.tester` in `.agentic.yml` (fall back to `models.default`, then the session default).
- Run: `printf '%s' 'baseline-<flow-slug>' > .claude/state/current_run.txt && printf '%s' 'baseline-<flow-slug>' > .claude/state/current_task.txt && printf '%s' 'tester' > .claude/state/current_role.txt && printf '%s' '<resolved_model>' > .claude/state/current_model.txt`  (current_run gives the tool calls a log context: `logs/baseline-<flow-slug>.jsonl`; current_model puts the model on each logged call)
- Invoke a **tester** subagent with that model (passed explicitly — do not rely on session inheritance) and:
...
- Run: `sh .claude/py .claude/hooks/lib/state.py clear`
