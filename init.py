...
def configure_dbhub(config: dict, config_path: Path, dry_run: bool):
    """
    ...
    print()
    print("  Requires: DBHub MCP server configured in ~/.claude/settings.json")
    print("  Supports: PostgreSQL, MySQL, MariaDB, SQL Server, SQLite")
    print()
    raw = input("  Enable DBHub integration? (yes / skip) [skip]: ").strip().lower()
    if raw != "yes":
        info("DBHub — skipped (enable later in .stangent/config.json)")
        return

    server_name = input("  MCP server name in your Claude Code settings [dbhub]: ").strip()
    server_name = server_name or "dbhub"
    ...
    ok(f"DBHub — configured (mcp_server: {server_name})")

    print(f"""
  ─────────────────────────────────────────────────────
  Next: register DBHub with Claude Code.

  1. Install DBHub (once):
       npm install -g @bytebase/dbhub

  2. Register via the Claude Code CLI (recommended — edits ~/.claude.json):
       claude mcp add --scope user {server_name} -- npx @bytebase/dbhub --transport stdio --dsn "YOUR_DSN"

     Replace YOUR_DSN with your actual connection string. Examples:
  ...
  ⚠  Common DSN gotchas:
     - Special chars in password must be URL-encoded:
         !→%21  @→%40  #→%23  $→%24  %→%25
     - Supabase: use the Session Pooler URL (IPv6 direct connections fail in Node):
         postgres://postgres.[ref]:[pass]@aws-0-[region].pooler.supabase.com:5432/postgres?sslmode=require
     - If you see an ESM/CJS crash on startup, apply this fix once:
         node -e "const p=require('path').join(require('os').homedir(),'AppData','Roaming','npm','node_modules','@bytebase','dbhub','node_modules','ssh-config','package.json'); const f=require('fs'); const j=JSON.parse(f.readFileSync(p)); j.type='module'; f.writeFileSync(p,JSON.stringify(j,null,2));"
       (Linux/Mac: adjust path to your global npm prefix)

  3. Restart Claude Code.

  DBHub will now be used automatically by the planner and query analyzer.
  ─────────────────────────────────────────────────────
""")
...
def run(args):
    ...
    configure_dbhub(config, config_path, dry_run)
